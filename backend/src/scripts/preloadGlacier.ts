...
import { fetchOSMDataForBounds } from '../services/osmService';
import { Pool } from 'pg';
...
function generateBoundingBoxGrid(bounds: typeof GLACIER_BOUNDS, chunkSize: number = 0.025) {
  const chunks = [];
...
async function preloadGlacierTrails() {
  ...
  for (const chunk of chunks) {
    try {
      console.log(`Processing chunk ${processed + 1}/${chunks.length}:`, 
        `${chunk.minLat.toFixed(3)},${chunk.minLng.toFixed(3)} to ${chunk.maxLat.toFixed(3)},${chunk.maxLng.toFixed(3)}`);
