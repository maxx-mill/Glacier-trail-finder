...
import { TrailSegment } from '../types/trail';
...
function isWay(element: OSMWay | OSMNode): element is OSMWay {
  return 'tags' in element;
}

function isNode(element: OSMWay | OSMNode): element is OSMNode {
  return 'lat' in element && 'lon' in element;
}

function classifyTrail(tags: OSMWay['tags']): { 
  ...
  if (tags.sac_scale) {
    ...
    if (sacScale === 'hiking') difficulty = 'Easy';
    else if (sacScale === 'mountain_hiking') difficulty = 'Moderate';
    else if (sacScale === 'demanding_mountain_hiking') difficulty = 'Hard';
    else if (sacScale === 'alpine_hiking') difficulty = 'Very Hard';
  } else if (tags.difficulty) {
    difficulty = tags.difficulty;
  } else {
    // Guess based on highway type
    const highway = tags.highway;
    if (highway === 'path' || highway === 'footway') difficulty = 'Easy';
    else if (highway === 'track') difficulty = 'Moderate';
    else if (highway === 'bridleway') difficulty = 'Easy';
  }
  ...
  if (tags.highway === 'cycleway' || bicycle === 'yes') trailType = 'Cycling';
  else if (tags.highway === 'bridleway') trailType = 'Horseback';
  else if (tags.route === 'hiking') trailType = 'Hiking';
...
async function fetchTileWithRetry(query: string, retryCount = 0): Promise<any> {
  try {
...
export async function fetchOSMDataForBounds(bounds: OSMBounds): Promise<void> {
  const { minLat, maxLat, minLng, maxLng } = bounds;
  
  // Check if we already have data for this area
  const existingData = await pool.query(
    `SELECT COUNT(*) FROM trail_segments 
     WHERE bounding_box @> $1::jsonb`,
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      const trails: Trail[] = ways.map(way => {
        const points: [number, number][] = Array.isArray(way.nodes)
          ? way.nodes
